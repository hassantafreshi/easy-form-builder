...
- [x] H17 — رفع باگ نمایش مقدارهای نوشته‌شده توسط logic در فیلد قابل‌مشاهده (`public/assets/js/conditional-logic-efb.js`، تابع `evaluate()`): `set_values`/`cleared_fields` به‌صورت **diff نسبت به rowهای submit** محاسبه می‌شوند. در حلقه چندپاسی evaluate، پاس ۱ مقدار محاسبه‌شده (مثلاً `calculate` روی Total) را داخل `sendBack` می‌نویسد؛ پاس ۲ همان مقدار را «مقدار اولیه» می‌بیند و diff خالی برمی‌گرداند — و چون `applyVisualState` فقط یک‌بار و با نتیجه **پاس آخر** اجرا می‌شود، input قابل‌مشاهده هرگز مقدار را نمی‌گرفت (داده در submit درست بود، فقط نمایش نه — دقیقاً علامت گزارش‌شده در تست دستی فرم Inspector Phase 7). همین مشکل برای `set_value`/`copy_value`/`clear_value` هم بود. رفع: انباشت نوشته‌های DOM در همه پاس‌ها (`domWrites`) و merge در نتیجه نهایی قبل از `applyVisualState`. تست: `tests/test-conditional-logic-validate-step.js` (T4.1–T4.3) — با برگرداندن موقت فیکس هر سه fail می‌شوند.
