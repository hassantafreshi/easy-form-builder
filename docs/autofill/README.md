# Auto-Populate for Easy Form Builder — Complete End-User Guide

> [Docs index](../README.md)

Stop making visitors retype information you already have. **Auto-Populate** is the Easy Form Builder add-on that automatically fills WordPress form fields — from a CSV dataset you upload, from the form's own previous submissions, or from a live external API — the moment a visitor enters a single lookup value like an email address or customer ID. This guide is verified directly against the add-on's code: installation from the Add-ons page, Dataset and Integrations setup, form-builder field mapping, front-end caching behavior, common errors, and a pre-launch checklist, available in four languages:
...
Looking for the **technical test guide** for the External API mode instead (test scenarios against public APIs, cache verification, error-case matrix)? See [testing/autofill-api](../testing/autofill-api/README.md) — that folder is written for QA/engineering, while this one is written for end users, content writers, and search engines.
