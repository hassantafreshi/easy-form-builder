...
function prev_btn_efb() {
  ...
  if(Number(valj_efb[0].show_icon)!=1){
      /* setProgressBar_efb() below recomputes every row from the step being
         moved to, but only for one of the new steps rows - a classic row still
         needs the step it is leaving cleared here, and so does a page where the
         shared runtime never loaded. */
...
/* The steps process and progress bar runtime moved to its own pair of files:
   includes/admin/assets/js/steps-progress-runtime-efb.js (the part a rendered
   form needs, which the front end inlines only when the form uses one of the
   new styles) and steps-progress-builder-efb.js (the markup builders, which
   only wp-admin needs). Both are enqueued whole in wp-admin. */
