             ...
             switch(l.id){
                 ...
                 if((typeof ajax_object_efm) == 'object'){
                    ...
                    const el_o = l.dataset.select.split(" @efb!")
                     ...
                     indx = sendBack_emsFormBuilder_pub.findIndex(x => x.id_ === l.dataset.vid);
                     if (indx == -1) {
                          sendBack_emsFormBuilder_pub.push(o[0]) 
                          
                     }else{ 
                        console.log(v , ajax_object_efm.text.selectOption)
                        if(v!=efb_var.text.selectOption){
                            console.log('after add multiselect');
                     ...
                     if(el_o.length>1){
                        ...
                        if (l.classList.contains('border-danger'))console.error('add border');
...
/*  multiSelect_selection_efb=(idSelected , idParent)=>{
    let l =  document.querySelector(`[data-row="${idSelected}"]`);
    let prnt = document.getElementById(`${idParent}_o`)
    prnt.dataset.select.length<1 ? prnt.innerHTML=e.dataset.name+"," :prnt.innerHTML+=e.dataset.name+","
    let fc = e.querySelector("TH")
    if (l.dataset.icon==1) fc.className="bi-check-square text-info efb";
    prnt.dataset.select+= `${idSelected} @efb!`;
   
   */ 
  
  if(typeof valj_efb!="undefined"){
    let ob =  valj_efb.find(x => x.id_ === l.dataset.vid); 
    const v= l.innerHTML.replaceAll(`,` , "@efb!");
    console.log(l.dataset.vid ,l);
    let o = [{ id_: l.dataset.vid, name: ob.name,amount:ob.amount, type:ob.type, value: v, session: sessionPub_emsFormBuilder }];   
