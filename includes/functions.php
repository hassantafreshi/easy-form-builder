...
class efbFunction {
	...
	public function text_efb($inp){
		...
		$lang = [
			...
			"rnfn" => $state  &&  isset($ac->text->rnfn) ? $ac->text->rnfn : __('Please update the file name to include only alphanumeric characters; special characters are not allowed.','easy-form-builder'),
	...
	/* public function efb_code_validate_create_old($fid, $type, $status, $tc) {
		...
		$query = $wpdb->prepare(
			"SELECT sid FROM {$table_name} WHERE ip = %s AND read_date > %s AND active = %d AND fid = %s",
			$ip,
			$date_now,
			1,
			$fid
