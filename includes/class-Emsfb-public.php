...
class _Public {
	...
	public function EMS_Form_Builder_track(){
		...
		*/
		...
		$track_content =  sprintf(
	 	...
	 	$content="<script>let sitekye_emsFormBuilder='' </script>".$s_m."
	...
	function public_scripts_and_css_head($state=''){
	  ...
	  public function get_form_public_efb($data_POST_) {
		...
		if ($fs != '') {
							...
							switch ($f['type']) {
								case 'email':
									...
									if (isset($item['value'])) {
										...
										if (isset($f['noti']) == true && intval($f['noti']) == 1)  $this->email_list_efb($email_user, 1, $item['value'], $e_ar);
										// end
									}
									$in_loop = false;
									break;
			...
			// if (true) {
				...
				if (is_array($formObj) && isset($formObj[0]['type'], $formObj[0]['captcha'])  && intval($formObj[0]['captcha']) == 1 && $formObj[0]['type'] != 'payment' && strlen($response) > 5) {
					//error_log('if captcha: ' .$formObj[0]['type'] , ' ' . $formObj[0]['captcha'] . ' ' . $response);
					if ($secretKey) {
				...
				error_log('captcha_success: ' . json_encode($captcha_success));
				if ( ($type != "logout" && $type != "recovery") && $not_captcha && ($captcha_success == "null" || $captcha_success->success != true)) {
					...
					if ($send_email_to_user_state) {
						...
						// $email_array_state = strpos($emailuser, ',') !== false;
						error_log('email list!');
						error_log(json_encode($emailuser));
						...
						case "form":
							...
							if ($send_email_to_user_state) {
							   ...
							   // error_log('after contet email: ' . $time);
								...
								error_log('email_user: ' . json_encode($email_user));
						...
						case "payment":
							...
							if ($value != null) {
								...
								if ($payment_getWay == "persiaPay") {
									$TokenCode = $setting['payToken'] ?? null;
								...
								foreach ($fs as $f) {
									...
									$valobj = empty($valobj) ? $it : array_merge($valobj, $it);
								...
								if ($send_email_to_user_state == true) {
									$state_email_user = $trackingCode_state == 1 ? 'notiToUserFormFilled_TrackingCode' : 'notiToUserFormFilled';
									$state_of_email = ['newMessage', $state_email_user];
									$msg_content = isset($formObj[0]['email_noti_type']) && $formObj[0]['email_noti_type'] == 'msg' ? str_replace("\"", "'", $this->email_get_content_efb($fs, $trackId)) : 'null';
									$msg_sub = $formObj[0]['email_sub'] ?? 'null';
							...
							case "register":
								...
								if (is_wp_error($state)) {
									...
									if ($email != "null") {
										...
										$check = $this->insert_message_db(0, false);
										$to = $email;
										// $email_array_state = strpos($email, ',') !== false;
										$this->email_list_efb($email_user, 1, $email, true);
										...
										if ($send_email_to_user_state == true) {
											$msg_sub = isset($formObj[0]['email_sub']) && $formObj[0]['email_sub'] != '' ? $formObj[0]['email_sub'] : 'null';
	  ...
	  public function get_track_public_api($data_POST_) {
		...
		error_log('get_track_public_api setting: ' . json_encode($setting));
		error_log('get_track_public_api data_POST: ' . json_encode($data_POST));

		error_log('get_track_public_api data_POST pl: ' .json_encode($captcha_success));
		 ...
		 if (!empty($captcha_success) &&$captcha_success->success==false &&  $not_captcha==false ) {
		  ...
		  wp_send_json_success($response, 200);
			...
			error_log('get_track_public_api table_name: ' . $table_name);
			error_log('get_track_public_api id: ' . $id);
			...
			$id;
				...
				foreach ($message as $k =>$f){
					...
					if($f->id_=='captcha_v2'){
						...
						if($stated==0){
							error_log('error 405: ' . $f->type . ' ' . $f->id_ . ' ' . $f->value);
				...
				$users_email =array();
				$valb=null;
				error_log('email actived:' . $email_actived);
				if($email_actived){
					// $email_noti_ids[] = $valn[0]['email_to'];
					foreach ($valn as $key => $value){
						if(isset($value['type']) &&  $value['type']=='email'  && isset($value['noti']) == true && intval($value['noti']) == 1 ){
							$email_noti_ids[] = $value['id_'];
						}
					}
					error_log('=========>noti email');
					error_log(json_encode($email_noti_ids));
					if(isset($id)){
						foreach ($msg_obj as $key => $value) {
							// if (isset($f['noti']) == true && intval($f['noti']) == 1)  $this->email_list_efb($email_user, 1, $item['value'], $e_ar);
							error_log(json_encode($value));
							if(isset($value['id_'])  &&  in_array($value['id_'] , $email_noti_ids)){
								error_log($value['value']);
	...
	function email_get_content_efb($content ,$track){
		...
		$total_amount =0;
		$lst = end($content);
		$formbuilderClass = null;

		function formbuilderClass_caller($pro){
			require_once(EMSFB_PLUGIN_DIRECTORY . '/includes/class-Emsfb-formbuilder.php');
			return new Formbuilder([], $pro);
		}
		$link_w = $lst['type']=="w_link" ? $lst['value'] : 'null';
		if(strlen($link_w)>5){
		...
		foreach ($content as $c){
			...
			if (is_string($c['value'])) {
			  ...
			  $name = substr($c['url'], strrpos($c['url'], '/') + 1, strrpos($c['url'], '.') - strrpos($c['url'], '/') - 1);
			   ...
			   if(isset($c['type']) && ($c['type'] == "Image" || $c['type'] == "image")){
				...
				if(empty($formbuilderClass)) $formbuilderClass = formbuilderClass_caller($this->pro_efb);
				$numberformat = $formbuilderClass->formatPrice_efb(number_format($c['price'], 0, '.', ','), $currency);
			...
			if ($c['type'] == "payment") {
			  ...
			  if ($c['paymentGateway'] == "stripe") {
				  if(empty($formbuilderClass)) $formbuilderClass = formbuilderClass_caller($this->pro);
				  $numberformat = $formbuilderClass->formatPrice_efb(number_format($c['paymentAmount'], 0, '.', ','), $c['paymentcurrency']);
	...
	public function email_list_efb( &$email_user , $pointer , $email , $state_array){
			$state_array= true;
			error_log('function >$this->email_list_efb');
			error_log( json_encode($email_user));
			error_log( 'poinster :' .$pointer);
			error_log( json_encode($email));
			error_log( 'is array ?' . $state_array);
