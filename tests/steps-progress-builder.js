...
async function run() {
  ...
  try {
    ...
    if (picker.steps) {
      check(picker.steps.options.join(',') === 'circles,pills,chevrons', 'steps picker offers three styles', picker.steps.options.join(','));
    ...
    if (picker.progress) {
      check(picker.progress.options.join(',') === 'bar,segments,ring', 'progress picker offers three styles', picker.progress.options.join(','));
    ...
    check(rejected === 'circles', 'an unknown style falls back to the original look', String(rejected));
    ...
    for (const c of cases) {
      ...
      if (await next.isVisible().catch(() => false)) {
        ...
        if (c.show_progress && c.progress_style === 'bar') check(s2.pct.trim() === want + '%', `${tag} percent follows`, s2.pct);
      }
