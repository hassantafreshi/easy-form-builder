...
(function (root, factory) {
  ...
  /* Fixed-point budget for the value loop. Rules run in priority order within a
   * single pass, so an acyclic cascade converges in one or two; a budget this
   * size only ever runs out on a chain wired against its own priority order, or
   * on a genuine loop — which is exactly what stabilized:false then reports. */
  var MAX_PASSES = 25;
  ...
  var MONEY_KEYS = ['paymentAmount', 'paid_amount', 'total', 'price', 'amount'];
  var MONEY_KEYS_PAYMENT = ['paymentAmount', 'paid_amount', 'total', 'price', 'value', 'amount'];
  ...
  function paymentState(fieldId, rows, values) {
    ...
    (rows || []).forEach(function (row) {
      ...
      /* Money keys in priority order, FIRST match wins. `amount` comes last on
       * purpose: in an EFB row `amount` is the field's ORDERING INDEX, and the
       * row a gateway pushes after a successful charge sets it to 0 while the
       * real figure sits in paymentAmount (and value). Reading `amount` as
       * money made every amount_gt false and every amount_lt true — a "small
       * payment" rule fired on a $10,000 charge. */
      var rowAmount = null;
      var keys = PAYMENT_TYPES.indexOf(type) !== -1 ? MONEY_KEYS_PAYMENT : MONEY_KEYS;
      keys.some(function (key) {
        if (!isNumericScalar(row[key])) return false;
        rowAmount = Number(row[key]);
        return true;
      });
      if (rowAmount !== null) amount = rowAmount;
    });
  ...
  function evaluateCondition(condition, values, structure, rows, env) {
    ...
    var normalizedExpected = normalizeValue(structure, fieldId, expected);

    if (Array.isArray(current)) {
      /* A multi-value field (checkbox, multiselect) holds one entry per ticked
       * option; the condition names exactly ONE of them. Two things made the
       * naive membership test always wrong:
       *   - normalizeValue() shapes its result like the FIELD, so a checkbox
       *     operand comes back wrapped: ['c1'].indexOf(['c1']) is -1.
       *   - the two sides can spell the same option differently — the builder
       *     stores the option's id_, a row records id_ob but falls back to the
       *     visible text when that is missing, and multiselect rows keep text.
      ...
      var wanted = [];
      var want = function (candidate) {
        if (candidate == null || candidate === '') return;
        var text = String(candidate).trim().toLowerCase();
        if (text !== '' && wanted.indexOf(text) === -1) wanted.push(text);
      };
      [].concat(normalizedExpected).forEach(want);
      want(expected);
      var optionsForField = indexStructure(structure).options[fieldId] || {};
      Object.keys(optionsForField).forEach(function (optionId) {
        /* both directions: the operand may be the option id (what the builder
         * writes) or its visible text (what an imported or hand-written rule
         * may carry), and a row may record either one. */
        if (optionId === String(expected)) want(optionsForField[optionId]);
        if (String(optionsForField[optionId]) === String(expected)) want(optionId);
      });

      var present = current.some(function (entry) {
        return wanted.indexOf(String(entry == null ? '' : entry).trim().toLowerCase()) !== -1;
      });
      if (operator === 'is') return present;
      if (operator === 'is_not') return !present;
      if (operator === 'is_empty') return current.length === 0;
      if (operator === 'is_not_empty') return current.length > 0;
      return compareScalar(current.join(' '), normalizedExpected, operator);
    }
    return compareScalar(current, normalizedExpected, operator);
  }
  ...
  function evaluatePass(structure, rows, rules, initialValues, env) {
    ...
    /* A hidden step ignores the fields that live on it. Step ACTION TARGETS are
     * step ids (`id_`), while a field's `step` is its POSITION — two namespaces
     * that are both small integers and routinely disagree (a form whose steps
     * were reordered has id_ "3" sitting at position 2). Resolving the target to
     * its position is therefore the only correct comparison: matching the raw id
     * against a position makes hiding step id_ "2" also ignore whatever happens
     * to sit at position 2, silently stripping that step's answers from the
     * submission. Only an unresolvable target — never produced by the builder,
     * which validates targets against real step ids — is read as a position. */
    var ignored = Object.assign({}, hidden, disabled);
    var hiddenStepPositions = {};
    ...
    Object.keys(index.fields).forEach(function (fieldId) {
      ...
      if (fieldStep && hiddenStepPositions[fieldStep]) ignored[fieldId] = true;
    });
  ...
  function evaluateDefinition(structure, rows, env) {
     ...
     * ({total} = {total} + {qty} climbs forever, never repeating a signature).
     * Only the budget catches the second, so exhausting it must report
     * stabilized:false — reporting true published the Nth iterate as if it were
     * a computed answer, which is how a "+1" rule silently produced +10. */
     ...
     * none: the visitor's own answers stand and the builder's Test Mode shows
     * the "did not stabilize" warning. Visibility/required state still applies
     * — those are set-membership and converge regardless of the value loop. */
    if (!stabilized) {
      result.values_map = clone(originalValues);
    }
